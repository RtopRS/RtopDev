...
/// Trait representing a Plugin. Must be implemented on your struct to declare it as a Plugin.
pub trait Plugin {
    /// Called every time Rtop need to update the plugin display
    fn display(&mut self, height: i32, width: i32) -> String;

    /// Called every time `display` is called, if the string returned is no null, Rtop will replace the widget title with the newly provided string
